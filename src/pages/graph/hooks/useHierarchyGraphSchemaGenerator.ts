...
export default function useHierarchyGraphSchemaGenerator() {
  const generateSchema = (data: any) => {
    ...
    for (let [path, pathData] of Object.entries(paths(data))) {
      rootNode.children.unshift({
        ...
        value: NODE_SIZE,
        id: nanoid(),
        ...getAdditionalData((pathData as any)[POST] || (pathData as any)[GET]),
        ...COLORS.pathNode,
        children: [],
      });

      const {
        extractedData: { requestBody, responses, parameters },
        entityName,
        entityDescription,
      ...
      const deps = [
        {
          ...
          description: entityDescription.requestBody,
        },
        {
          ...
          description: entityDescription.responses,
        },
        {
          name: "parameters",
          list: parameters,
  ...
  const extractDataByMethod = (pathData: any, method: METHODS) => {
    ...
    const { requestBodyDepsParameters, responsesDepsParameters } =
      extractParameters({
        requestBodyDeps,
        responsesDeps,
        parametersDepsList,
    ...
    return {
      ...
      entityDescription: {
        requestBody: requestBodyDeps?.description,
        responses: responsesDeps?.description,
      },
    };
  };
    ...
    if (propertyContent.$ref) {
      ...
      if (entityContent?.properties) {
        ...
        for (let [propsName, propsContent] of propsEntities) {
          ...
          const entityName = extractEntityName(propertyData.$ref);

          ref.unshift({
            name: entityName || propsName,
            id: nanoid(),
  ...
  const getUniversalResponseEntityContent = (responsesDeps: any) => {
    const propertiesResponsesDepsRefsList = [];

    const universalResponseEntityContent: any = extractDataByRef(
      responsesDeps.allOf[0].$ref
    );

    const propertiesResponsesDeps = Object.entries(
      universalResponseEntityContent.properties
    );
    ...
    return propertiesResponsesDepsRefsList;
  };
  ...
  const extractParameters = ({
    ...
    const requestBodyData =
      requestBodyDeps?.properties || requestBodyDeps?.items;

    if (requestBodyData) {
      const propertiesRequestBody = Object.entries(requestBodyData);

      for (let [propsName, propsContent] of propertiesRequestBody) {
        if (typeof propsContent === "string") {
          const entityName = extractEntityName(propsContent);

          const ref = {
            name: entityName,
            id: nanoid(),
            ...getAdditionalData(requestBodyData),
            children: [],
          };

          extractOtherDeps(requestBodyData, propsName, ref.children);
    ...
    let propertiesResponsesDepsRefsList: any = [];
    if (responsesDeps.properties) {
      ...
      for (let [propsName, propsContent] of propertiesResponsesDeps) {
        ...
        let ref = {
          ...
          children: [...getUniversalResponseEntityContent(responsesDeps)],
        };
      ...
      propertiesResponsesDepsRefsList =
        getUniversalResponseEntityContent(responsesDeps);
    }
