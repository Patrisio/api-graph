...
export default function useHierarchyGraph(graphData: any) {
  const [currentPointerIndex, setCurrentPointerIndex] = useState<number | null>(
    null
  );
  const [foundEntitiesCount, setFoundEntitiesCount] = useState<number | null>(
    null
  );
  const [prevHighlightedNodes, setPrevHighlightedNodes] = useState<
    { color: string; id: string; scrollTop: number }[]
  >([]);
  ...
  const { generateSchema } = useHierarchyGraphSchemaGenerator();

  const getScrollTop = (node: any) => {
    return (
      (node as Element)?.getBoundingClientRect().y +
  ...
  const handleFoundNodeByName = (
    nodeData: any,
    foundNodeDepsIds: any
  ): boolean => {
    ...
    if (foundNodeChildren.length > 0) {
      recursiveTraverse(foundNodeDepsIds, foundNodeChildren);
    }
  ...
  const handleGraphNodes = (foundNode: any) => {
    ...
    const filteredNodes = d3.selectAll("text").filter((d: any): any => {
  ...
  const resetGraph = () => {
    ...
    setPrevHighlightedNodes([]);
    disableAnimationToSelectedCircleNode(
      prevHighlightedNodes[currentPointerIndex as number].id
    );
  };
  ...
  useEffect(() => {
    if (prevHighlightedNodes.length > 0 && currentPointerIndex !== null) {
      ...
      if (currentPointerIndex > 0)
        disableAnimationToSelectedCircleNode(
          prevHighlightedNodes[currentPointerIndex - 1].id
        );
