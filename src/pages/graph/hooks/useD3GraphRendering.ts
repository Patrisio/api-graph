...
import { MARGINS } from "../common/constants";

export default function useD3GraphRendering(
  ...
  const setDimensionsAndMarginsToGraph = (
    graphWidth: number,
    graphHeight: number
  ) => {
    let { margin } = graphData;

    graphData.dimensions.width = graphWidth - margin.left - margin.right; // 1800
    graphData.dimensions.height = graphHeight; //35000 - margin.top - margin.bottom;
  ...
  const assignDataToGraph = useCallback(() => {
  ...
  const renderGraph = () => {
    setDimensionsAndMarginsToGraph(graphWidth, graphHeight);
    assignDataToGraph();
